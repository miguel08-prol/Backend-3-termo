        ...
        <main :class="sidebarOpen ? 'ml-72' : 'ml-24'" class="flex-1 transition-all duration-300">
            ...
            <header class="h-24 flex items-center justify-between px-10 bg-white/80 backdrop-blur-md border-b border-slate-100 sticky top-0 z-40">
                <div data-aos="fade-right">
                    <h2 class="text-2xl font-black text-slate-900 tracking-tight italic">Gestão de Clientes</h2>
                </div>
                
                <div class="flex items-center gap-4" data-aos="fade-left">
                    <a href="{{ route('clientes.create') }}" class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center gap-2">
            ...
            <div class="p-10">
                <div class="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden" data-aos="fade-up">
                    <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-slate-900 italic">Base de Clientes</h3>
                        <span class="px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest italic">Total: {{ $clientes->total() }} cadastros</span>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-slate-50">
                            <thead>
                                <tr class="bg-slate-50/50">
                                    <th class="px-8 py-5 text-left text-xs font-black text-slate-400 uppercase tracking-widest">Identificação</th>
                                ...
                                <tr class="hover:bg-indigo-50/30 transition-all group">
                                    <td class="px-8 py-6">
                                        <div class="flex items-center gap-4">
                                            <div class="w-12 h-12 bg-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 shadow-sm overflow-hidden">
                                                <img src="https://ui-avatars.com/api/?name={{ urlencode($cliente->nome) }}&background=EEF2FF&color=4F46E5&bold=true" class="w-full h-full object-cover">
                                            </div>
                                            <div>
                                                <div class="text-sm font-bold text-slate-900">{{ $cliente->nome }}</div>
                                                <div class="text-[10px] text-emerald-500 font-black uppercase tracking-widest">Cliente Ativo</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="text-sm font-medium text-slate-600">{{ $cliente->cpf }}</div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="text-sm font-bold text-slate-900">{{ $cliente->email }}</div>
                                        <div class="text-xs text-slate-400 font-medium">{{ $cliente->telefone }}</div>
                                    </td>
                                    <td class="px-8 py-6 text-center">
                                        <div class="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                            <a href="{{ route('clientes.edit', $cliente->id) }}" class="w-10 h-10 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all shadow-sm">
                                                <i class="ph ph-pencil-simple text-xl"></i>
                                            </a>
                                            <button onclick="openDeleteModal('{{ $cliente->id }}', '{{ $cliente->nome }}')" class="w-10 h-10 bg-red-50 text-red-600 rounded-xl flex items-center justify-center hover:bg-red-600 hover:text-white transition-all shadow-sm">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative bg-white rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl transition-all border border-slate-100">
                <div class="w-20 h-20 bg-red-50 text-red-500 rounded-[1.5rem] flex items-center justify-center mx-auto mb-6">
                    <i class="ph-fill ph-user-minus text-4xl"></i>
                </div>
                <h3 class="text-2xl font-black text-slate-900 text-center mb-2 tracking-tighter italic">Remover Cliente?</h3>
                <p class="text-slate-500 text-center text-sm mb-10 font-medium leading-relaxed">Você está prestes a excluir <span id="clientNameSpan" class="font-bold text-slate-900 underline decoration-red-200"></span>. Todos os dados de medidas associados serão perdidos.</p>
                ...
                <div class="flex gap-4">
                    ...
                    <form id="deleteForm" method="POST" class="flex-1">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full py-4 bg-red-500 text-white rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-red-600 shadow-lg shadow-red-100 transition">Sim, Excluir</button>
                    </form>
                </div>
            </div>
        </div>
