        ...
        <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div class="order-2 lg:order-1">
                ...
                <div class="flex items-center justify-between mb-4 flex-wrap gap-4">
                    <div>
                        <h1 id="poke-name" class="text-6xl md:text-8xl font-extrabold capitalize tracking-tighter">
                            {{ $pokemon['name'] }}
                        </h1>
                ...
                <div class="no-scrollbar pr-4">
                    ...
                    <!-- NOVA TAB: LISTA DE POKÉMON COM FILTROS -->
                ...
                <div class="mt-10 flex gap-4">
                    ...
                    @if($prevId)
                    <a href="{{ url('/pokemon/' . $prevId) }}" class="glass hover:bg-white/10 text-white font-bold py-4 px-8 rounded-2xl transition-all active:scale-95 flex items-center gap-2 border-white/5">
                    ...
                    @if($nextId)
                    <a href="{{ url('/pokemon/' . $nextId) }}" class="bg-blue-600 hover:bg-blue-500 text-white font-black py-4 px-10 rounded-2xl transition-all shadow-xl shadow-blue-900/30 active:scale-95 flex-1 text-center">
                        PRÓXIMO
                    </a>
                    @else
...
@if(isset($pokemon['is_custom']) && $pokemon['is_custom'])
<div class="custom-buttons mt-4 flex justify-center gap-4">
    <a href="{{ route('custom-pokemons.edit', str_replace('custom_', '', $pokemon['id'])) }}" class="text-purple-400 hover:text-purple-300 text-sm font-semibold transition flex items-center gap-1">
    ...
    <footer class="fixed bottom-0 w-full hidden md:block py-6 glass border-t-0 border-white/5">
        <div class="max-w-6xl mx-auto flex justify-around text-center">
            <div>
                ...
                <p class="text-xl font-bold">{{ isset($pokemon['height']) ? number_format($pokemon['height'] / 10, 1) : 0 }}m</p>
            </div>
            <div>
                <p class="text-gray-500 uppercase font-bold text-[10px] mb-1 tracking-wider">PESO</p>
                <p class="text-xl font-bold">{{ isset($pokemon['weight']) ? number_format($pokemon['weight'] / 10, 1) : 0 }}kg</p>
            </div>
            <div>
                <p class="text-gray-500 uppercase font-bold text-[10px] mb-1 tracking-wider">EXPERIÊNCIA BASE</p>
                <p class="text-xl font-bold">{{ $pokemon['base_experience'] ?? 0 }}</p>
            </div>
        </div>
    </footer>
...
async function loadPokemons() {
    ...
    function goToPokemon(identifier, isCustom) {
        if (isCustom) {
            ...
            window.location.href = `/pokemon/${identifier}`;
        }
    }
...
function checkCustomButtonsVisibility() {
    ...
    if (customButtons && activeTab) {
        // Esconde os botões se a tab ativa for 'lista'
