...
<body>
                                ...
                                <div class="evolution-card glass-card p-5 rounded-2xl">
                                    <div class="flex items-center gap-4">
                                        <div class="relative">
                                            @php
                                                $evoImage = $evo['image'] ?? '';
                                                if ($evoImage && !str_starts_with($evoImage, 'http') && !str_starts_with($evoImage, 'storage')) {
                                                    $evoImage = asset($evoImage);
                                                } elseif ($evoImage && str_starts_with($evoImage, 'storage')) {
                                                    $evoImage = asset($evoImage);
                                                }
                                                if (!$evoImage) {
                                            ...
                                            @endphp
                                            <img id="evo-preview-{{ $index }}" src="{{ $evoImage }}" 
                                                 class="w-20 h-20 rounded-full object-cover bg-black/30 cursor-pointer image-upload-area p-1">
                                            <input type="file" class="evo-image-input hidden" accept="image/*" data-target="evo-preview-{{ $index }}" data-index="{{ $index }}">
                ...
                <div class="text-center w-full">
                    ...
                    <div class="relative group">
                        @php
                            ...
                            if ($mainImageUrl && !str_starts_with($mainImageUrl, 'http') && !str_starts_with($mainImageUrl, 'storage')) {
                                $mainImageUrl = asset($mainImageUrl);
                            } elseif ($mainImageUrl && str_starts_with($mainImageUrl, 'storage')) {
                                $mainImageUrl = asset($mainImageUrl);
                            }
                            if (!$mainImageUrl) {
                                $mainImageUrl = 'https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/0.png';
                            }
                        @endphp
                    ...
                    <div class="glass rounded-2xl p-6 mt-8">
                        <div class="flex justify-around text-center">
                            <div>
                                ...
                                <p class="text-sm text-gray-400"><span id="height-m">{{ number_format(($customPokemon->height ?? 17) / 10, 1) }}</span> metros</p>
                            </div>
                            <div>
                                ...
                                <p class="text-sm text-gray-400"><span id="weight-kg">{{ number_format(($customPokemon->weight ?? 905) / 10, 1) }}</span> kg</p>
                            </div>
                            <div>
    ...
    <script>
        ...
        confirmDeleteBtn?.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    showToast('🗑️ Pokémon excluído com sucesso! Redirecionando...', 'success');
                    setTimeout(() => {
                        window.location.href = result.redirect_url;
                    }, 1500);
                } else {
                    showToast(result.message || 'Erro ao excluir Pokémon', 'error');
                ...
                console.error('Erro:', error);
                showToast('Erro ao excluir Pokémon: ' + error.message, 'error');
                confirmDeleteBtn.disabled = false;
                confirmDeleteBtn.innerHTML = 'Sim, Excluir';
                closeDeleteModal();
            }
        ...
        // FUNÇÃO PARA UPLOAD DE IMAGEM DE EVOLUÇÃO
        ...
        // EVOLUÇÕES DINÂMICAS
        let evolutionIndex = {{ count($evolutions) }};
        ...
        updateBtn?.addEventListener('click', async () => {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    showToast('✅ Pokémon atualizado com sucesso! Redirecionando...', 'success');
                    setTimeout(() => {
                        window.location.href = result.redirect_url;
                    }, 1500);
                } else {
                    showToast(result.message || 'Erro ao atualizar Pokémon', 'error');
                    updateBtn.disabled = false;
                    updateBtn.innerHTML = '💾 ATUALIZAR POKÉMON';
                    updateBtn.classList.remove('opacity-70');
                }
                ...
                console.error('Erro:', error);
                showToast('Erro ao atualizar Pokémon: ' + error.message, 'error');
                updateBtn.disabled = false;
                updateBtn.innerHTML = '💾 ATUALIZAR POKÉMON';
                updateBtn.classList.remove('opacity-70');
            }
