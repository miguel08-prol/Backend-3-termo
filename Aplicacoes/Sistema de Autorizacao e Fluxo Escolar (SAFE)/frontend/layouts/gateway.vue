  ...
  <div class="min-h-screen flex bg-slate-50 dark:bg-slate-950 font-sans">
    ...
    <header class="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 z-40 flex items-center justify-between px-4 shadow-sm">
      <div class="flex items-center gap-3">
        ...
        <div class="font-bold text-slate-800 dark:text-white tracking-tight text-lg flex items-center gap-2">
          SAFE
          <span class="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-bold border border-emerald-500/20">Portaria</span>
        </div>
      </div>
    </header>
    ...
    <aside :class="[
      ...
      <div class="flex items-center justify-between h-20 px-4 shrink-0">
        <div class="flex items-center gap-3">
          <div class="min-w-[2.25rem] h-9 rounded-lg bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center font-bold text-white shadow-[0_0_15px_rgba(16,185,129,0.3)] border border-emerald-400/20">
            S
          </div>
          <div :class="[
            ...
            <span class="text-emerald-400 text-[11px] font-bold tracking-widest uppercase mt-1">Portaria</span>
          </div>
        </div>
      </div>
      ...
      <nav class="flex-1 overflow-y-auto overflow-x-hidden py-4 space-y-8 custom-scrollbar">
        ...
        <div class="px-3">
          ...
          <ul class="space-y-1">
            <li>
              <NuxtLink to="/gateway/dashboard" class="nav-item" active-class="nav-active">
                <div class="nav-icon">
            ...
            <li>
              <NuxtLink to="/gateway/exits" class="nav-item" active-class="nav-active">
                <div class="nav-icon">
                  <Icon name="heroicons:arrow-right-start-on-rectangle-20-solid" class="w-5 h-5" />
                </div>
                <span :class="['nav-text', isMobileMenuOpen ? 'opacity-100' : 'opacity-0 group-hover:opacity-100']">Saídas Pendentes</span>
              </NuxtLink>
            </li>
          </ul>
        </div>
        ...
        <div class="px-3">
          ...
          <ul class="space-y-1">
            <li>
              <NuxtLink to="/gateway/history" class="nav-item" active-class="nav-active">
                <div class="nav-icon">
                  <Icon name="heroicons:clock-20-solid" class="w-5 h-5" />
                </div>
                <span :class="['nav-text', isMobileMenuOpen ? 'opacity-100' : 'opacity-0 group-hover:opacity-100']">Registro de Saídas</span>
              </NuxtLink>
            </li>
          </ul>
        </div>
...
.nav-active {
  @apply bg-emerald-500/10 text-white shadow-[inset_3px_0_0_0_#10b981];
}
