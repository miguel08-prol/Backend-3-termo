...
class NotificationService
{
    ...
    public function notifyProfessor(Authorization $authorization, User $professor): void
    {
        $subject = "🔔 Nova Autorização de Saída - {$authorization->aluno_nome}";
        $body = "Olá Professor(a),\n\nO aluno {$authorization->aluno_nome} da turma {$authorization->turma} solicitou autorização para saída às " .
               date('H:i', strtotime($authorization->horario_saida)) . ". Acesse o sistema para aprovar ou rejeitar.";
        
        $this->sendEmail($professor->email, $subject, $body, $authorization);
        
        if ($professor->telefone) {
            $whatsappMsg = "📋 Nova solicitação de saída: {$authorization->aluno_nome} - {$authorization->turma} às " . date('H:i', strtotime($authorization->horario_saida));
    ...
    {
        $subject = "✅ Autorização Validada - {$authorization->aluno_nome}";
