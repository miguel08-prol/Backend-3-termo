...
<head>
    ...
    <title>CineDex — Galeria de Filmes</title>
    ...
    <style>
        ...
        /* Botão de Favoritar (Estilo Home) */
        .btn-fav {
        ...
        .movie-card:hover .btn-fav { opacity: 1; transform: translateY(0); }
...
<body class="pb-32">
    ...
    <header class="w-full px-6 md:px-12 py-10 flex flex-col md:flex-row justify-between items-center gap-8">
        <div class="reveal">
            <h1 class="text-4xl font-black italic tracking-tighter uppercase leading-none">
                EXPLORE <span class="text-rose-600">FILMES</span>
            </h1>
            <div class="flex items-center gap-2 mt-2">
                <div class="h-1 w-12 bg-rose-600 rounded-full"></div>
                <p class="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em]">{{ $tituloPagina ?? 'Catálogo Completo' }}</p>
            </div>
        </div>

        <form action="{{ route('filmes.index') }}" method="GET" class="w-full md:w-[450px]">
            <div class="relative group">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Pesquisar por título ou gênero..." 
    ...
    <div class="px-6 md:px-12 mb-12 flex gap-4 overflow-x-auto no-scrollbar py-2">
        @foreach(['Todos', 'Ação', 'Aventura', 'Animação', 'Comédia', 'Terror', 'Drama', 'Sci-Fi'] as $cat)
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-6 gap-y-10">
            @forelse($movies as $movie)
                <div class="movie-card relative">
                    <button onclick="toggleFav('{{ $movie['id'] }}', '{{ addslashes($movie['title']) }}', '{{ $movie['poster_path'] }}', this)" 
                            class="btn-fav fav-btn-check" data-id="{{ $movie['id'] }}">
                    ...
                    <a href="{{ route('detalhes', ['type' => 'movie', 'id' => $movie['id']]) }}" class="block glass-card rounded-[2rem] overflow-hidden">
                        ...
                        <div class="p-5">
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                ...
                                <span class="text-[9px] text-gray-500 font-bold uppercase">• Filme</span>
                            </div>
                        </div>
                    </a>
                </div>
            @empty
                <div class="col-span-full py-32 flex flex-col items-center opacity-40">
                    ...
                    <p class="font-black uppercase italic tracking-widest">Nada encontrado</p>
                </div>
            @endforelse
        </div>
    </main>
    ...
    <div class="fixed bottom-8 left-1/2 -translate-x-1/2 bg-black/40 backdrop-blur-2xl border border-white/10 px-10 py-5 rounded-[2.5rem] flex gap-12 z-50 shadow-2xl">
        ...
        <a href="#" class="text-rose-500 flex flex-col items-center gap-1 group">
            <svg class="w-6 h-6 scale-110" fill="currentColor" viewBox="0 0 24 24"><path d="M7 4v16l13-8z"/></svg>
            <span class="text-[9px] font-black tracking-widest uppercase">Filmes</span>
        </a>
        <a href="#" class="text-gray-500 hover:text-white transition-all flex flex-col items-center gap-1 group">
            <svg class="w-6 h-6 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2.5"/></svg>
            <span class="text-[9px] font-black tracking-widest uppercase">Séries</span>
        </a>
    </div>

    <script>
        // Lógica de Favoritos (Sincronizada com a Home)
        ...
        // Checar quais filmes já estão favoritados ao carregar a página
        function checkFavs() {
